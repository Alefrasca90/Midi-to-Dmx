...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.mappings = {} # Struttura: {"cc_7": [1, 2, 3]}
        self.scenes = {} 
        self.active_scene = None
        self.is_learning = False
        self.midi_input = None

        self.setWindowTitle("MIDI-DMX Studio: Mapping Control")
        ...
        controls.addWidget(QLabel("<b>2. SELEZIONE E LIVE</b>"))
        ...
        self.btn_rec = QPushButton("REC SCENE")
        self.btn_rec.clicked.connect(self.record_scene)
        self.btn_rec.setStyleSheet("background-color: #c0392b; font-weight: bold;")
        controls.addWidget(self.btn_rec)
        
        self.scene_list = QListWidget()
        self.scene_list.itemClicked.connect(self.activate_scene_item)
        ...
        self.btn_blackout.setStyleSheet("background-color: #8e44ad; font-weight: bold; padding: 10px;")
    ...
    def record_scene(self):
        name = f"Scena {len(self.scenes) + 1}"
        current_frame = {str(i): self.dmx.output_frame[i] for i in range(1, 513) if self.dmx.output_frame[i] > 0}
        if not current_frame: return
        self.scenes[name] = current_frame
        self.scene_list.addItem(name)
        self.save_data()

    def activate_scene_item(self, item):
        name = item.text()
        if self.active_scene == name:
            ...
            for ch, val in self.scenes[name].items():
                new_buf[int(ch)] = val
    ...
    def midi_callback(self, msg):
        if self.is_learning:
            ...
            sig_id = None
            if msg.type == 'control_change': sig_id = f"cc_{msg.control}"
            elif msg.type in ['note_on', 'note_off']: sig_id = f"note_{msg.note}"
            
            if sig_id and sig_id in self.mappings:
                val = int(msg.value * 2.007) if msg.type == 'control_change' else (255 if msg.velocity > 0 else 0)
                for ch in self.mappings[sig_id]:
                    self.dmx.live_buffer[ch] = val
    ...
    def update_ui(self):
        ...
        for i in range(512):
            ...
            # Testo: Verde/Arancione SOLO se mappato MIDI, altrimenti Grigio scuro
            if is_map:
            ...
            # SFONDO FISSO: Sempre nero, non cambia con lo slider
